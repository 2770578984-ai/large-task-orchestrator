...
同一任务因外部信息缺失而等待时，补充已请求的信息就会触发重查并继续已授权工作，无需再次调用技能或另说“继续”。只解除部分依赖时，其余阻塞只影响相关步骤。用户要求保持暂停、取消任务或用量限制仍优先遵守。

任务文档分别记录工作进度、宿主目标真实状态、恢复能力和待处理的宿主操作。如果会话继续但目标仍 blocked，技能会立即解释区别和所需恢复入口，保留原目标及有效验收证据；不会在每次回复重复同一个未变化的恢复提示。

## 持久目标与能力限制

部分 Codex 宿主暴露 `get_goal`、`create_goal` 和 `update_goal`。技能读取当前工具契约，在显式调用上下文中建立目标，并读回真实状态；不会虚构接口或自行设置预算。已有未完成目标、暂停与用量限制仍受保护。

**自动创建与自动恢复是两种能力。** 有获准调用的恢复操作时，技能恢复原目标并读回确认；只有上述三个工具时，`update_goal` 不能改回 active，需要宿主目标控制。Codex 桌面可使用目标栏的“恢复”；支持相应命令的入口可用 `/goal resume`。技能继续当前会话内已授权的工作，并及时说明持久接续仍需要这一次宿主操作。仅补充信息不能保证目标自动恢复。参见[官方目标控制](https://learn.chatgpt.com/docs/long-running-work)。

只有实际观察到无需用户再发消息的后续目标轮次及必要工作，才记录自动接续已验证；单独读回 active 不足以证明。没有运行中的轮次时，本技能无法自行发现外部信息已变化或唤醒自己，也不会自行安装监控。
...
使用[恢复流程评测](tests/RECOVERY.md)及其合成情境检查缺少恢复接口、部分信息到位、用户暂停、用量限制、读回失败和提示处理。模拟决策检查与真实宿主调度实测分开记录；模拟器不能证明当前宿主具有自动恢复接口。

此前这项评测只测试了修订版，没有证明它优于旧版。现提供[有状态 A/B 评测](tests/evals/README.md)，让独立子 agent 执行不同提交的技能版本，再盲评实际本地产物与工具调用。实测结果和限制见[对照报告](reports/recovery-ab-evaluation.md)。评测资源只用于开发，不会安装到运行时技能目录。
