...
你描述初始需求，回答确实需要你决定的问题。Codex 先调查项目，在实施前向你说明对目标、范围和验收的理解，再填写任务文档并推进已授权的实施与验证。
...
- 实施前向用户说明目标、范围、验收、关键依据及未决事项；重构时对照新要求与旧行为。
...
启动前的信息对齐会明确展示给你，不能仅保存在任务文件中。已有充分决定时，说明依据后自动执行；关键产品取舍仍不明确时，集中询问并等待必要答复，其余调查与准备继续。旧系统有某个功能不代表必须保留，新设计没有画出某个功能也不等于授权删除；先依据你的明确决定和任务材料确定，仍有歧义才问。恢复同一任务时只补充新差异，不重复要求批准整个计划。
...
另有[启动前信息对齐评测](tests/evals/alignment/README.md)，通过隔离配置项目检查实施前说明、必要问题、明确决定及仅分析边界。[本轮对照报告](reports/alignment-evaluation.md)区分实际观察结果、小样本限制和本地通信代理的限制。
