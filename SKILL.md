...
On recovery, read `STATE`, applicable AGENTS.md instructions, and the latest user updates. Verify the directory, branch or working-tree changes, and runtime facts; then read goal state and continue at the first executable unfinished step. Do not rely on old completion marks when their evidence is no longer valid, or repeat questions about recorded decisions that remain valid. Preserve ownership of other tasks' changes.
